...
async function executeTool(name: string, input: Record<string, unknown>, locale: Locale) {
  const supabase = await createClient();

  if (name === 'search_perfumes') {
    let query = supabase
      .from('perfumes')
      .select('id, name, olfactive_family, gender_target, community_rating, key_accords, brands(name), translations')
    ...
    // Return locale-aware evokes if available
    return (data ?? []).map(p => ({
      ...p,
      evokes: locale !== 'en'
        ? ((p.translations as any)?.[locale]?.evokes ?? p.key_accords)
        : p.key_accords,
    }));
  }
  ...
  if (name === 'get_perfume_details') {
    const { data, error } = await supabase
      .from('perfumes')
      .select('*, brands(name), translations')
      .eq('id', input.id)
      .single();

    if (error) return { error: error.message };

    // Merge locale translations over English fields
    const localeData = locale !== 'en'
      ? (data?.translations as any)?.[locale] ?? {}
      : {};
...
export async function POST(
  ...
  const language = LOCALE_LANGUAGE_NAMES[(locale as Locale) ?? 'es'];
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          const response = await anthropic.messages.create({
            ...
            system: `You are a knowledgeable perfume and fragrance expert for Aroma, an AI-powered fragrance discovery platform.
Always respond in ${language}.
Use the available tools to search the database before making recommendations.
Always explain your reasoning in ${language}.
Be precise, knowledgeable, and passionate about fragrance.`,
          });
